...
#pragma endregion OUTMATRIX
        ...
        // Unary operators
            ...
            const Vector3& Get_Right() { return *((Vector3*)&this->m[MATROW_RIGHT][0]); }
            const Vector3& Get_Left() { return *((Vector3*)&this->m[MATROW_RIGHT][0]) * -1.f; }
            const Vector3& Get_Up() { return *((Vector3*)&this->m[MATROW_UP][0]); }
            const Vector3& Get_Down() { return *((Vector3*)&this->m[MATROW_UP][0]) * -1.f; }
            const Vector3& Get_Look() { return *((Vector3*)&this->m[MATROW_LOOK][0]); }
            const Vector3& Get_Back() { return *((Vector3*)&this->m[MATROW_LOOK][0]) * -1.f; }
            const Vector3& Get_Pos() { return *((Vector3*)&this->m[MATROW_POS][0]); }
...
namespace std
{
    ...
    template<> struct less<DirectX::PTH::RECTANGLE>
    {
        bool operator()(const DirectX::PTH::RECTANGLE& r1, const DirectX::PTH::RECTANGLE& r2) const noexcept
        {
            return ((r1.x < r2.x)
                || ((r1.x == r2.x) && (r1.y < r2.y))
    ...
    template<> struct less<DirectX::PTH::Color>
    {
        bool operator()(const DirectX::PTH::Color& C1, const DirectX::PTH::Color& C2) const noexcept
        {
            return ((C1.x < C2.x)
                || ((C1.x == C2.x) && (C1.y < C2.y))
    ...
    template<> struct less<DirectX::PTH::Viewport>
    {
        bool operator()(const DirectX::PTH::Viewport& vp1, const DirectX::PTH::Viewport& vp2) const noexcept
        {
