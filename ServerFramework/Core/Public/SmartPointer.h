		...
		*/
		...
		public:
			...
			*/
			...
			T* get() const { return ptr.load(std::memory_order_relaxed).get(); }
...
namespace Core {
	...
	template<class T>
	using SHPTR = __declspec(align(Core::PTH::CASH_ALGIN_VALUE)) PTH::USharedPtr<T>;
	...
	template<class T>
	using WKPTR = __declspec(align(Core::PTH::CASH_ALGIN_VALUE))   PTH::UWeakPtr<T>;
...
#define CACHE_ALGIN  __declspec(align(Core::PTH::CASH_ALGIN_VALUE)) 
...
namespace std
{
	...
	*/
	template <typename T>
	struct hash<Core::PTH::USharedPtr<T>> {
		size_t operator()(const Core::PTH::USharedPtr<T>& ptr) const {
	...
	*/
	template <typename T>
	struct equal_to<Core::PTH::USharedPtr<T>> {
		bool operator()(const Core::PTH::USharedPtr<T>& lhs, const Core::PTH::USharedPtr<T>& rhs) const {
	...
	*/
	template <typename T>
	struct less<Core::PTH::USharedPtr<T>> {
		bool operator()(const Core::PTH::USharedPtr<T>& lhs, Core::PTH::USharedPtr<T>& rhs) const {
			return lhs.get() < rhs.get();
		}
	};
}
