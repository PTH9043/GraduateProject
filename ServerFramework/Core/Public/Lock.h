...
namespace Core {
	...
	namespace Lock {
		...
		*/
...
#ifdef USE_DEBUG
			ReadLockGuard(const URWLock& _Lock, const char* _name) : m_Lock{ _Lock }, m_Name{ _name } { m_Lock.ReadLock(_name); }
		...
		public:
#ifdef USE_DEBUG
			WriteLockGuard(const URWLock& _Lock, const char* _name) : m_Lock{ _Lock }, m_Name{ _name } { m_Lock.WriteLock(_name); }
