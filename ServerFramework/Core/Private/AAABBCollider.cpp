...
namespace Core {
	...
	AAABBCollider::AAABBCollider(const Vector3 _vOriPos, const Vector3 _vOriScale) : 
		ACollider(COLLIDERTYPE::AABB, _vOriPos, _vOriScale)
	{
	}

	void AAABBCollider::Tick(const _matrix _WorldMatrix)
	{
	...
	_bool AAABBCollider::IsCollision(SHPTR<ACollider> _spCollider)
	{
