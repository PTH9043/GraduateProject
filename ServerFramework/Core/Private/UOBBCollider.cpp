...
namespace Core {
	...
	UOBBCollider::UOBBCollider(const Vector3 _vOriPos, const Vector3 _vOriScale) : 
		UCollider(COLLIDERTYPE::OBB, _vOriPos, _vOriScale)
	{
	}

	void UOBBCollider::Tick(const _matrix _WorldMatrix)
	{
	...
	_bool UOBBCollider::IsCollision(SHPTR<UCollider> _spCollider)
	{
	...
	_float UOBBCollider::ProjectionOBBToAxis(const Vector3& _vPos, const Vector3& _vScale,
