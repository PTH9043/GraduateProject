    ...
    CSHPTRREF<URenderObject> _spGraphicRenderObject)
{
    ...
    m_spGraphicrenderObject = _spGraphicRenderObject;
...
HRESULT URenderTargetManager::AddRenderTargetGroup(const RTGROUPID& _eGroupID, const std::vector<RTDESC>& _rtVec)
{
    ...
    for (const RTDESC& iter : _rtVec) {
        ...
        tTarget.pTexture = CreateNative<UTexture>(m_spGraphicrenderObject->GetDevice(), iter);
    ...
    SHPTR<URenderTargetGroup> pRtGroups = CreateNative<URenderTargetGroup>
        (m_spGraphicrenderObject->GetDevice(), rtGroups, m_spDepthStencilTexture);
...
void URenderTargetManager::OmSetRenderTargets(CSHPTRREF<URenderObject> _spRenderObject, const RTGROUPID& _eGroupID, const _uint _index, const _uint _iOffset)
{
    OmSetRenderTargets(_spRenderObject->GetCommand(), _eGroupID, _index, _iOffset);
}

void URenderTargetManager::OmSetRenderTargets(CSHPTRREF<URenderObject> _spRenderObject, const RTGROUPID& _eGroupID)
{
    OmSetRenderTargets(_spRenderObject->GetCommand(), _eGroupID);
}

void URenderTargetManager::ClearRenderTargetView(CSHPTRREF<URenderObject> _spRenderObject, const RTGROUPID& _eGroupID, const _uint _index)
{
    ClearRenderTargetView(_spRenderObject->GetCommand(), _eGroupID, _index);
}

void URenderTargetManager::ClearRenderTargetView(CSHPTRREF<URenderObject> _spRenderObject, const RTGROUPID& _eGroupID)
{
    ClearRenderTargetView(_spRenderObject->GetCommand(), _eGroupID);
}

void URenderTargetManager::WaitTargetToResource(CSHPTRREF<URenderObject> _spRenderObject, const RTGROUPID& _eGroupID)
{
    WaitTargetToResource(_spRenderObject->GetCommand(), _eGroupID);
}

void URenderTargetManager::WaitResourceToTarget(CSHPTRREF<URenderObject> _spRenderObject, const RTGROUPID& _eGroupID)
{
    WaitResourceToTarget(_spRenderObject->GetCommand(), _eGroupID);
}
...
void URenderTargetManager::OnResizeWindow(CSHPTRREF<UGraphicDevice> _spGraphicDevice)
{
    ...
    {
        iter.second->RemakeRenderTargets(m_spGraphicrenderObject->GetDevice(), m_spDepthStencilTexture, m_spGraphicDesc);
    }
    CreateDefaultRenderTargets(_spGraphicDevice);
...
void URenderTargetManager::CreateDepthStencilTexture()
{
    ...
    m_spDepthStencilTexture = CreateNative<UTexture>(m_spGraphicrenderObject->GetDevice(), stRtDesc);
}
...
void URenderTargetManager::CreateDefaultRenderTargets(CSHPTRREF<UGraphicDevice> _spGraphicDevice)
{
    ...
    // SwapChain Group
    {
        ...
        for (_uint i = 0; i < SWAP_CHAIN_END; ++i)
        {
            ...
            rtVec[i].pTexture = CreateNative<UTexture>(m_spGraphicrenderObject->GetDevice(), resource);
            rtVec[i].SetColor(_float4(0.f, 0.f, 255.f, 1.f));
        }
        // CreateToMethod
        SHPTR<URenderTargetGroup> pRtGroups = CreateNative<URenderTargetGroup>(m_spGraphicrenderObject->GetDevice(),
