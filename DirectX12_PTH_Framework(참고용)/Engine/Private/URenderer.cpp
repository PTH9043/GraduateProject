...
void URenderer::RenderRTs()
{
    ...
    for (auto& iter : m_arrActiveDrawRenderList[RENDERID::RI_TARGETS])
    {
        ...
        for (auto& Mirror : iter.second)
        {
            ...
            SHPTR<URenderObject> spRenderObject = spGameInstance->GetRenderObject((GRAPHICRENDEROBJECT_TYPE)iMirror);
            ...
            Mirror->Render(spRenderObject->GetCommand(), spRenderObject->GetTableDescriptor());
...
{
    SHPTR<UGameInstance> spGameInstance = GET_INSTANCE(UGameInstance);
    SHPTR<URenderObject> spRenderObject = spGameInstance->GetRenderObject(GRAPHICRENDEROBJECT_TYPE::BLEND);
    {
        SHPTR<UGameInstance> spGameInstance = GET_INSTANCE(UGameInstance);
        spRenderObject->BeginRenderTareget(spGameInstance->GetD3DViewport(), spGameInstance->GetD3DWindowSizeRect());
        m_spPipeLine->BindViewProjMatrix(spRenderObject->GetCommand());
    ...
    spShader->SettingPipeLineState(spRenderObject->GetCommand());
    spShader->SetTableDescriptor(spRenderObject->GetTableDescriptor());
    ...
    m_spVIBufferRect->Render(spShader, spRenderObject->GetCommand());
    spRenderTargetGroup->WaitTargetToResource(spRenderObject);
    spRenderObject->ExcuteCommand();
}
