...
HRESULT UVIBufferTerrain::NativeConstruct(const TERRAINDESC& _stTerrainDesc, const _float& _fHeightSize, D3D_PRIMITIVE_TOPOLOGY _eTopology)
{
	RETURN_CHECK_FAILED(NativeConstruct(), E_FAIL);
	{
		...
		for (_uint i = 0; i < m_stTerrainDesc.iNumVerticiesZ; ++i)
		{
			for (_uint j = 0; j < m_stTerrainDesc.iNumVerticiesX; ++j)
			{
				...
				VertexNormal[iIndex].vNormal = _float3(0.f, 1.f, 0.f);
		...
		RETURN_CHECK_FAILED(CreateVtxBuffer(VERTEX_CNT, sizeof(VTXNORMAL), VertexNormal.data(),
			_eTopology, VertexPos), E_FAIL);
	}

	return S_OK;
}

HRESULT UVIBufferTerrain::NativeConstruct(const _wstring& _wstrMapFilePath, const _float& _fHeightSize, D3D_PRIMITIVE_TOPOLOGY _eTopology)
{
	RETURN_CHECK_FAILED(NativeConstruct(), E_FAIL);
...
HRESULT UVIBufferTerrain::NativeConstruct(CSHPTRREF<URawData> _spRawData, const _float& _fHeightSize, D3D_PRIMITIVE_TOPOLOGY _eTopology)
{
