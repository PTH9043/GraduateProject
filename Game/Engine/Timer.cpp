...
CGameTimer::CGameTimer() {
	...
	if (::QueryPerformanceFrequency((LARGE_INTEGER*)&m_nPerformanceFrequency)) {
		...
		m_fTimeScalse = 1.0f / m_nPerformanceFrequency;
	}
	else {

		m_bHardwareHasPerformanceCounter = FALSE;
		//m_nLastTime = ::timeGetTime();
		m_fTimeScalse = 0.001f;//m_fTimeScale
	}
	m_nSampleCount = 0;
...
void CGameTimer::Tick(float fLockFPS)
{
	...
	float fTimeElapsed = (m_nCurrentTime - m_nLastTime) * m_fTimeScalse;//m_fTimeScale
	if (fLockFPS > 0.0f)
	{
		...
		while (fTimeElapsed < (1.0f / fLockFPS))
		{
			...
			//마지막으로 이 함수를 호출한 이후 경과한 시간을 계산한다. 
			fTimeElapsed = (m_nCurrentTime - m_nLastTime) * m_fTimeScalse;//m_fTimeScale
		}
	}
